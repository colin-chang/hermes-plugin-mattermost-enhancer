...
下面是这个项目（插件 + 配套脚本）修掉的 7 个 Bug。每个 Bug 都附带了**造成的影响**，这样你能判断是否遇到过这些问题。
...
| **6** | AI 提问不显眼：Hermes 向你提问（多选或开放问题）时，只发一行纯文本，混在对话流里容易被错过 | 你没看到问题 → 没回复 → AI 卡住超时 → 同时触发会话分裂，AI 答非所问 💀 | 提问渲染为带按钮的交互卡片，醒目且可点击 |
| **7** | 会话分裂（AI 答非所问）：AI 在等待你回复提问时，你发的下一条消息被当成"新对话"开启，AI 完全失忆 | 你在 Thread A 里跟 AI 聊得好好的，突然 AI 不认得你了，开始瞎答 | 消息正确路由给等待中的 AI，不再创建新会话 |
...
上面 Bug 表格里的 **Bug #4**（DM 审批缺少 user_id）、**Bug #5**（工具进度消息不进 Thread）和 **Bug #7**（会话分裂），就是插件够不到的地方。

### 配套脚本是干什么的？

就是修这三个插件够不到的 Bug。它直接修改了 Hermes 的源码文件，加了少量代码。其中 Bug #7（会话分裂）也已[提交到上游](https://github.com/NousResearch/hermes-agent/pull/30669)，将来 Hermes 官方版本可能会自带修复。

![Patch脚本运行效果](images/patch.webp)

### 我该装哪个？

**两个都要装。** 先装插件（实现主要功能），再跑脚本（修三个底层 Bug）。
